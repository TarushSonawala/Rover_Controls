...
controller.init()
output_string = " M{Gear}X{left_joystick_x}Y{left_joystick_y}P{right_joystick_x}Q{right_joystick_y}A{A}S{trigger}R{resetValue}E"
# Set up the socket
    ...
    while True:
        ...
        while running:
            ...
            if timer >= interval:
                ...
                right_joystick_x_0 = (
                    map(controller.get_axis(AXIS_RIGHT_STICK_X), -1, 1, 2, -2))
                right_joystick_x = str(right_joystick_x_0)
                right_joystick_y_0 = (
                    map(controller.get_axis(AXIS_RIGHT_STICK_Y), -1, 1, -2, 2))
                right_joystick_y = str(right_joystick_y_0)

                if int(trigger) >= 0:
                    right_trigger_axis = (
                        map(controller.get_axis(TRIGGER_RIGHT), -1, 1, 0, 2))
                    trigger = str(right_trigger_axis)
                if int(trigger) <= 0:
                    left_trigger_axis = (
                        map(controller.get_axis(TRIGGER_LEFT), -1, 1, 0, -2))
                    trigger = str(left_trigger_axis)
                data = output_string.format(
                    Gear=Gear, left_joystick_x=left_joystick_x, left_joystick_y=left_joystick_y, right_joystick_x=right_joystick_x, right_joystick_y=right_joystick_y, A=A, trigger=trigger, resetValue=resetValue, E=0)
                print(data)
